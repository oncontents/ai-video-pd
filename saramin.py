...
class SaraminScraper(BaseScraper):
    ...
    def fetch(self) -> list[JobPosting]:
        ...
        for keyword in self.settings.keywords:
            ...
            for item in soup.select(".item_recruit, .list_item, tr"):
                text = clean_text(item.get_text(" "))
                if keyword not in text:
                    continue
                link = item.select_one("a[href*='/zf_user/jobs/relay/view'], a[href]")
                title = clean_text(link.get_text(" ")) if link else text[:80]
                company = item.select_one(".corp_name, .company_nm, .str_tit")
                ...
                posting = JobPosting(
                    ...
                    posted_at=None,
                    url=absolutize("https://www.saramin.co.kr", link.get("href") if link else None),
                )
